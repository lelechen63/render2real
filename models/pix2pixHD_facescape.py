...
class Pix2PixHDFewhsotModel(BaseModel):
    def name(self):
        return 'Pix2PixHDFewhsotModel'
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G_fewshot(netG_input_nc, opt.output_nc, opt.ngf, opt.netG, 
                                      opt.n_downsample_global, opt.n_blocks_global, opt.n_local_enhancers, 
                                      opt.n_blocks_local, opt.norm, gpu_ids=self.gpu_ids)        
...
class InferenceFewshotModel(Pix2PixHDFewhsotModel):
    def forward(self, renderred_image, eye_parsing  = None):
        return self.inference(renderred_image, eye_parsing)
